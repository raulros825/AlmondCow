...
function mtInitSirv() {
    ...
    for (var i in SirvVariants) {
        ...
        .replace(/{product\-sku}/g, (typeof SirvVariantsSKU[i].sku != 'undefined' && SirvVariantsSKU[i].sku!='' && SirvVariantsSKU[i].sku!=null)?SirvVariantsSKU[i].sku:i)
            ...
            success: function( data ) {
                ...
                if (this.spinID == currentVariantID) {
                    if (SirvSpinPosition == 'first') {
                        ...
                        jQuery('.Sirv').attr('data-src', SirvVariants[this.spinID]+$h);
                    }
                  currentVariantID = 0;
                }
            }  
...
function mtInitSirvIcon(spinURL) {
    ...
    jQuery('.MagicToolboxSlides').append(
        '<div data-slide-id="spin" class="MagicToolboxSlide"><div class="Sirv" id="sirv-spin" data-src="' + spinURL + $h + '"></div></div>'
    );
